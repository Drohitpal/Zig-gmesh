...
#include "gps_reader.h"   /* ← GPS + compass */
...
void esp_zb_app_signal_handler(esp_zb_app_signal_t *signal_struct)
{
    ...
    case ESP_ZB_BDB_SIGNAL_STEERING:
        if (err_status == ESP_OK) {
            /* Wait 0.5 s then start the periodic GPS send loop */
            esp_zb_scheduler_alarm((esp_zb_callback_t)send_value_cb, 0, 500);
        } else {
            esp_zb_scheduler_alarm(
                (esp_zb_callback_t)bdb_start_top_level_commissioning_cb,
                ESP_ZB_BDB_MODE_NETWORK_STEERING, 1000);
...
void app_main(void)
{
    ...
    /* Start GPS + compass reader BEFORE Zigbee task so data is
       available as soon as the first send_value_cb fires. */
    gps_reader_init();
